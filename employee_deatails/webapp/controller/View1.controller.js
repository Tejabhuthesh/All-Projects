sap.ui.define([
    ...
    function (Controller,JSONModel, MessageToast, FilterOperator, Filter, MessageBox, exportLibrary, Spreadsheet) {
        ...
        return Controller.extend("idED.employeedeatails.controller.View1", {
            onInit: function () {
           
                var oModel = this.getOwnerComponent().getModel();
                // oModel.read("/EmployeeDeatailsSet('123')?$expand=EmployeeData,EmployeeDeatailsTOEmployeeCOLLEGE&", {
                    oModel.read("/EmployeeDeatailsSet", {
                ...
                this._oAppView = this.getOwnerComponent().getModel();
            ...
            onAddButtonMatnrPress: function (oEvent) {
                // let aData = this._oAppView.getProperty("/EmployeeDeatailsSet");
                // aData = aData.length > 0 ? Array.from(aData) : [];
                // aData.push({});
                // this._oAppView.setProperty("/results", aData);

                var aData = oEvent.getSource().getModel("Data1").getData().results;
                aData = aData.length > 0 ? Array.from(aData) : [];
                aData.push({});
                this.getView().getModel("Data1").oData.results = aData;
                // setData(aData);
            },
    ...
    // ------------------------------------------------------------------------------------
            ...
            onSearch: function (evt) {
                ...
                if (sQuery) {
                    var oFilter = new Filter('EmployeeId', 'Contains', sQuery);                   //EQ =
                    var oFilter1 = new Filter('EmployeeName', 'Contains', sQuery);                // CONTAINS Notequal
                    var oFilter2 = new Filter('EmployeeMob', 'Contains', sQuery);
                    var oFilter3 = new Filter('EmployeeSalary', 'Contains', sQuery);
                    var oFilter4 = new Filter('EmployeeCollegename', 'Contains', sQuery);
